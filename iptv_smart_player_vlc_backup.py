...
class SmartIPTVPlayer:
    ...
    def launch_video_player(self, stream_data, env):
        ...
        try:
            ...
            time.sleep(1)  # Reduced from 2 seconds for faster restarts
    ...
    def launch_vlc(self, stream_url, env):
        ...
        try:
            ...
            # Quick Pi detection (cached to avoid repeated checks)
            if not hasattr(self, '_pi_detection_done'):
                try:
                        ...
                        if 'BCM' in cpuinfo or 'Raspberry Pi' in cpuinfo:
                            ...
                            try:
                                ...
                                if model_info.returncode == 0:
                                    pi_model = model_info.stdout.strip().replace('\x00', '')
                                    logging.info(f"   Detected: {pi_model}")
                                    if 'Pi 3' in pi_model or 'Pi Zero' in pi_model:
                                        logging.info("   Note: Older Pi model - using optimized lightweight settings")
                            except Exception:
                                pass
                except Exception:
                    pass
            ...
            # Quick network connectivity test (only on first run)
            if not hasattr(self, '_network_checked'):
                try:
                    import socket
                    test_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    test_socket.settimeout(1)  # Reduced from 2 seconds
            ...
            for i, base_cmd in enumerate(vlc_configs, 1):
                if i == 1:
                    ...
                    if platform.system() == 'Windows':
                        ...
                        # Raspberry Pi 3: OPTIMIZED lightweight settings
                        # Pi 3 Model B has limited RAM (1GB) and older CPU - be conservative but faster
                        pi_args = [
                            '--network-caching=800',     # Reduced from 1000ms for faster startup
                            '--live-caching=150',        # Reduced from 200ms for lower latency
                            '--file-caching=150',        # Reduced from 200ms for responsiveness
                            '--clock-jitter=0',          # Eliminate timing jitter
                            '--no-audio-time-stretch',   # Prevent audio lag
                        ]
                        
                        # Check Pi model - disable MMAL on Pi 3 to prevent lockups
                        # Use cached model info
                        if pi_model:
                            # Pi 3 and older: Use software decode (more stable)
                            if 'Pi 3' in pi_model or 'Pi Zero' in pi_model or 'Pi 2' in pi_model:
                                pi_args.extend(['--avcodec-hw=none'])  # Software decode prevents lockups
                                logging.info("   Using software decode (stable for Pi 3)")
                            # Pi 4 and newer: Can try hardware decode
                            else:
                                try:
                                    ...
                                    if gpu_mem.returncode == 0 and 'gpu=' in gpu_mem.stdout:
                                        ...
                                        if mem_value >= 128:
                                            pi_args.extend(['--avcodec-hw=mmal'])
                                            logging.info("   Using MMAL hardware decode")
                                        else:
                                            pi_args.extend(['--avcodec-hw=none'])
                                            logging.info("   Using software decode (low GPU memory)")
                                except Exception:
                                    pi_args.extend(['--avcodec-hw=none'])
                                    logging.info("   Using software decode (safe default)")
                        else:
                            # Default to software decode if detection fails
                            pi_args.extend(['--avcodec-hw=none'])
                            logging.info("   Using software decode (safe default)")
                        
                        performance_args.extend(pi_args)
                        logging.info("   Applied Raspberry Pi optimized settings (faster startup, stable)")
                    else:
                    ...
                    if self.vlc_version_info['major'] >= 3:
                        ...
                        if is_raspberry_pi:
                            # Pi 3: Minimal frame options to prevent lockups
                            # Avoid aggressive frame dropping that can overload CPU
                            general_args.extend([
                    ...
                    # Second attempt: Minimal stable config for Pi 3 (faster startup)
                    performance_args = [
                        '--network-caching=600',   # Lower latency, faster startup
                        '--http-user-agent', 'Mozilla/5.0 (Smart-IPTV-Player)',
                        '--no-video-title-show',
                        '--quiet',
                        '--no-snapshot-preview',
                    ...
                    # Add minimal caching options if supported
                    if self.vlc_version_info['supports_advanced_caching']:
                        performance_args.extend([
                            '--live-caching=100',      # Very minimal live buffering for speed
                            '--file-caching=100',      # Very minimal file buffering
                        ])
                    
                    # For Pi 3: Use software decode only (MMAL causes lockups)
                    if is_raspberry_pi:
                        performance_args.extend([
                            '--avcodec-hw=none',       # Software decode for stability
                        ])
                        logging.info("   Using software decode for stability")
                    # For other systems: Try hardware decode
                    elif self.vlc_version_info['supports_modern_hw_decode']:
                        hw_decode = 'any'
                        performance_args.extend([
                            '--codec=avcodec',
                            f'--avcodec-hw={hw_decode}',
                        ])
                    
                    # Add minimal frame management for non-Pi systems
                    if not is_raspberry_pi and self.vlc_version_info['major'] >= 3:
                        performance_args.extend([
                            '--drop-late-frames',
                            '--skip-frames',
                        ])
                    ...
                    # Fallback: Conservative but still faster than defaults
                    performance_args = [
                        '--network-caching=1000',  # Better than default 3000, balanced
                        '--http-user-agent', 'Mozilla/5.0 (Smart-IPTV-Player)',
                        '--no-video-title-show',
                        '--quiet',
                        '--no-snapshot-preview',
                ...
                # Reduced delay between attempts for faster failover
                if i < len(vlc_configs):  # Only wait if there are more configs to try
                    logging.info("   Waiting 1.5 seconds before next config...")
                    time.sleep(1.5)
    ...
    def _start_vlc_process(self, cmd, env, config_name):
        ...
        try:
            ...
            if platform.system() != 'Windows':
                # Linux-specific: Start VLC with NORMAL priority for Pi 3 stability
                # (High priority can cause lockups on resource-constrained systems)
                def setup_process():
                    os.setsid()
                    # For Pi 3: Use default priority to prevent CPU starvation
                    # of other critical processes
            ...
            # Monitor startup with better crash detection (OPTIMIZED)
            logging.info(f"[LOADING] Quick startup check for VLC {config_name}...")
            
            # Initial rapid check for immediate crashes (0.5s)
            time.sleep(0.5)
            ...
            # Shorter stability check - 3 seconds total is enough (vs 12 seconds)
            # Pi 3 needs time to initialize but 12 seconds was excessive
            for i in range(3):
                if self.current_process.poll() is not None:
                    ...
                    try:
                        ...
                        pass
                    logging.warning(f"[FAIL] VLC {config_name} crashed after {i+1} seconds (exit code: {self.current_process.returncode})")
                    return False
            ...
            logging.info("   Stream startup time: ~3.5 seconds (optimized)")
            return True
    ...
    def start_smart_player(self):
        ...
        time.sleep(2)  # Reduced from 5 seconds - Pi 3 is stable faster than expected
        ...
        try:
            while self.running:
                ...
                # Check system resources every 5 minutes on Pi to prevent lockups
                check_count += 1
                if check_count % 5 == 0 and platform.system() != 'Windows':
                    try:
                        # Check available memory
                        mem_info = subprocess.run(['free', '-m'], capture_output=True, text=True, timeout=2)
                        if mem_info.returncode == 0:
                            ...
                            if len(lines) > 1:
                                ...
                                if len(mem_parts) >= 7:
                                    ...
                                    if available_mb < 100:
                                        logging.warning(f"[WARNING] Low memory: {available_mb}MB available - consider restarting")
                    except Exception:
                        pass  # Don't fail if resource check fails
